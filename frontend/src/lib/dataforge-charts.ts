...
export function guessNumericCol(
  rows: DataRow[],
  cols?: string[]
): string | null {
...
export function guessCategoricalCol(
  rows: DataRow[],
  cols?: string[]
): string | null {
  const c = cols ?? columnsOf(rows)
...
export function aggregateCategory(
  ...
  const c =
    ...
    cols.find(
      (col) =>
        !["id", "date", "customer_id", "txn_id", "ndc_code"].includes(
          col.toLowerCase()
...
export function scatterPoints(
  ...
  const x =
    xCol ?? SCATTER_X_HINTS.find((h) => numCols.includes(h)) ?? numCols[0]
  const y =
    yCol ??
  ...
  const label =
    labelCol ??
    cols.find((c) =>
      ["product", "drug", "category", "merchant", "region", "segment"].includes(
        c.toLowerCase()
      )
    ) ??
    null
...
export function waterfallSegments(
  ...
  const demandShift =
    Math.round((variance + Math.abs(stockoutLoss)) * 100) / 100
  return [
    { name: "Prior Baseline", base: 0, span: priorRev, kind: "total" },
    {
      name: "Stockout Losses",
      base: currentRev - demandShift,
      span: -Math.abs(stockoutLoss),
      kind: "down",
    },
    {
      name: "Demand Shift",
...
export function simulationCurve(
  grossRecovery: number,
  holdingIncrease: number
): SimulationCurve {
  const buffers = [
    "7-Day (Lean)",
    "14-Day (Baseline)",
  ...
  const gross = [0.35, 0, 1, 1.18, 1.26].map(
    (f) => Math.round(grossRecovery * f * 100) / 100
  )
  const holding = [0.25, 0, 1, 2.35, 3.85].map(
    (f) => Math.round(holdingIncrease * f * 100) / 100
  )
  return {
    buffers,
    gross,
    holding,
    net: gross.map((g, i) => Math.round((g - holding[i]) * 100) / 100),
  }
}
...
export function buildNetwork(
  ...
  ordered.forEach((members, ci) => {
    ...
    visible.forEach((id, i) => {
      ...
      const radius =
        ci === 0 && visible.length === 1 ? 0 : ringGap * (ci + 1) * 0.85
      nodes.push({
        id,
        degree: degree.get(id) ?? 0,
        centrality:
          Math.round(((degree.get(id) ?? 0) / Math.max(n - 1, 1)) * 10000) /
          10000,
        community: ci,
